  ...
  return (
    <div className="flex items-center">
      ...
      {[...Array(fullStars)].map((_, i) => (
        <FaStar key={i} size={size} className="text-yellow-400 mr-1" />
      ))}

      {/* Yarım yıldız */}
      {halfStar && <FaStarHalfAlt size={size} className="text-yellow-400 mr-1" />}

      {/* Boş yıldızlar */}
      {[...Array(emptyStars)].map((_, i) => (
        <FaRegStar key={i} size={size} className="text-gray-300 mr-1" />
      ))}
