...
import { ChevronDown, Search, MapPin, Plus, Minus, ChevronLeft, ChevronRight } from "lucide-react";
...
function DatePicker() {
  // State management
  const [personCount, setPersonCount] = useState<PersonCount>({ adults: 1, children: 0 });
  ...
  const popularCities = [
    ...
    { name: "Marseille", region: "Provence-Alpes-Côte d'Azur, France", icon: MapPin },
    { name: "Rabat", region: "Rabat-Salé, Fas", icon: MapPin },
  ];

  // Person count handlers
  const updatePersonCount = (type: 'adults' | 'children', operation: 'increment' | 'decrement') => {
    setPersonCount(prev => {
  ...
  const isDateStartOrEnd = (date: Date) => {
    if (!dateRange.checkIn || !dateRange.checkOut) return false;
    return date.getTime() === dateRange.checkIn.getTime() || 
  ...
  const handleDropdownChange = (dropdownName: keyof DropdownStates, isOpen: boolean) => {
      ...
      <CardHeader>
        <div className="grid grid-cols-5 gap-7">
          ...
          <div className="flex flex-col border-r border-gray-200 pr-7">
            ...
            <CardDescription>
              <DropdownMenu onOpenChange={(open) => handleDropdownChange('city', open)}>
                <DropdownMenuTrigger>
                ...
                <DropdownMenuContent className="w-80 p-0">
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-800 mb-3">Popüler aramalar</h3>
          ...
          <div className="flex flex-col border-r border-gray-200 pr-7">
            ...
            <CardDescription>
              <DropdownMenu onOpenChange={(open) => handleDropdownChange('checkIn', open)}>
                <DropdownMenuTrigger>
                  <div className="flex flex-row gap-5 mt-2">
                    <p className="font-normal">
                      {dateRange.checkIn ? dateRange.checkIn.toLocaleDateString('tr-TR', { 
                        day: 'numeric', 
                        month: 'long' 
                      }) : "Tarih Seçin"}
                    </p>
                ...
                <DropdownMenuContent className="w-[600px] p-0">
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-gray-800">Giriş tarihi seçin</h3>
                    ...
                    <div className="grid grid-cols-2 gap-8">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-7 gap-1">
                          {['Su', 'Pzt', 'Sal', 'Çar', 'Per', 'Cum', 'Cmt'].map(day => (
                            ...
                            return (
                              <button
                                ...
                                className={`
                                  ...
                                  ${isInRange && !isStartOrEnd ? 'bg-blue-100 text-blue-600' : ''}
                                  ${isCurrentMonth && !isInRange ? 'hover:bg-gray-100 text-gray-700' : ''}
                                `}
                              >
                                {date.getDate()}
                              </button>
                      ...
                      <div>
                        <h4 className="text-center font-medium text-gray-700 mb-3">
                          {new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1).toLocaleDateString('tr-TR', { month: 'long', year: 'numeric' })}
                        </h4>
                        <div className="grid grid-cols-7 gap-1">
                          {['Su', 'Pzt', 'Sal', 'Çar', 'Per', 'Cum', 'Cmt'].map(day => (
                            <div key={day} className="text-center text-sm text-gray-500 py-2">
                          ...
                          {generateCalendarDays(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1)).map((date, index) => {
                            const isCurrentMonth = date.getMonth() === (currentMonth.getMonth() + 1) % 12;
                            const isInRange = isDateInRange(date);
                            const isStartOrEnd = isDateStartOrEnd(date);
          ...
          <div className="flex flex-col border-r border-gray-200 pr-7">
            ...
            <CardDescription>
              <DropdownMenu onOpenChange={(open) => handleDropdownChange('checkOut', open)}>
                <DropdownMenuTrigger>
                  <div className="flex flex-row gap-5 mt-2">
                    <p className="font-normal">
                      {dateRange.checkOut ? dateRange.checkOut.toLocaleDateString('tr-TR', { 
                        day: 'numeric', 
                        month: 'long' 
                      }) : "Tarih Seçin"}
                    </p>
                ...
                <DropdownMenuContent className="w-[600px] p-0">
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-gray-800">Çıkış tarihi seçin</h3>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
          ...
          <div className="flex flex-col">
            ...
            <CardDescription>
              <DropdownMenu onOpenChange={(open) => handleDropdownChange('personCount', open)}>
                <DropdownMenuTrigger>
                  <div className="flex flex-row gap-5 mt-2">
                    <p className="font-normal">
                ...
                <DropdownMenuContent className="w-80 p-0">
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-800 mb-4">Popüler aramalar</h3>
                    
                    {/* Adults Counter */}
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="font-medium text-gray-800">Yetişkin 16+ yaş</p>
                      </div>
                      <div className="flex items-center border border-gray-200 rounded-lg">
                        <button
                          onClick={() => updatePersonCount('adults', 'decrement')}
                        ...
                        <div className="w-12 h-10 flex items-center justify-center border-x border-gray-200">
                          <span className="text-gray-800 font-medium">{personCount.adults}</span>
                        </div>
                        <button
                          onClick={() => updatePersonCount('adults', 'increment')}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-gray-800">Çocuk 0-16 yaş</p>
                      </div>
                      <div className="flex items-center border border-gray-200 rounded-lg">
                        <button
                          onClick={() => updatePersonCount('children', 'decrement')}
                          className="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-50 rounded-l-lg"
                        >
                          <Minus className="w-4 h-4" />
                        </button>
                        <div className="w-12 h-10 flex items-center justify-center border-x border-gray-200">
                          <span className="text-gray-800 font-medium">{personCount.children}</span>
                        </div>
                        <button
                          onClick={() => updatePersonCount('children', 'increment')}
                          className="w-10 h-10 flex items-center justify-center text-blue-600 hover:bg-gray-50 rounded-r-lg"
                        >
                          <Plus className="w-4 h-4" />
                        </button>
          ...
          <div className="flex flex-col">
            <Button 
              className="bg-blue-600 hover:bg-blue-700 text-white py-5 font-bold rounded-lg transition-colors duration-200 flex items-center gap-2"
            >
