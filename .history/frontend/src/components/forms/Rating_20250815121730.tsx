  ...
  return (
    <div className="flex items-center">
      ...
      {[...Array(fullStars)].map((_, i) => (
        <Star key={i} size={size} className="text-yellow-400 mr-1" />
      ))}

      {/* Yarım yıldız */}
      {halfStar && <StarHalf size={size} className="text-yellow-400 mr-1" />}

      {/* Boş yıldızlar */}
      {[...Array(emptyStars)].map((_, i) => (
        <Star key={i} size={size} className="text-gray-300 mr-1" />
      ))}
